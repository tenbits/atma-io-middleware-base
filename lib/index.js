...
var io = globalIo && globalIo.File ? globalIo : require('atma-io');
exports.io = io;
;
...
var CacheProvider = (function () {
    ...
    CacheProvider.prototype.getSync = function (file) {
        ...
        if (item == null || item.isBusy()) {
            item = this.Items[path] = new CacheItemSync(path, file.content);
        }
        return item;
    };
    CacheProvider.prototype.getAsync = function (file) {
        ...
        var item = this.Items[path];
        if (item == null) {
            item = this.Items[path] = new CacheItemAsync(path, file.content);
        }
        return item;
    };
...
var CacheItem = (function (_super) {
    __extends(CacheItem, _super);
    function CacheItem(path, inputContent) {
        var _this = _super.call(this) || this;
        _this.path = path;
        _this.inputContent = inputContent;
        _this.isCached = false;
        _this.error = null;
        _this.inputHash = Hashable.doHash(inputContent);
        _this.load();
        return _this;
    }
    CacheItem.prototype.write = function (result) {
        if (result == null || typeof result !== 'string' || result === '') {
            return;
        }
        this.outputContent = result;
        this.isCached = true;
        return dependencies_1.io.File.writeAsync(this.path, this.serialize());
    };
    ...
    CacheItem.prototype.onCacheFileLoaded = function (content) {
        var _a = this.deserialize(content), hash = _a[0], result = _a[1];
...
var CacheItemAsync = (function (_super) {
    ...
    CacheItemAsync.prototype.load = function () {
        ...
        dependencies_1.io.File.existsAsync(path).then(function (exists) {
            ...
            dependencies_1.io.File.readAsync(path).then(function (content) {
                _this.onCacheFileLoaded(content);
            }, function (error) {
                _this.onError(error);
            });
        });
...
var CacheItemSync = (function (_super) {
    ...
    CacheItemSync.prototype.load = function () {
        ...
        try {
            var content = dependencies_1.io.File.read(path);
