...
module.exports = {
    create: function create(logOptions) {
        var type = logOptions == null ? null : logOptions.type;
        switch (type) {
            case 'silent':
                return new NoLogger();
            case 'file':
...
var ILogger = class_create({
    constructor: function constructor(logOptions) {
        this.options = logOptions;
    },
    lock: function lock() {},
    release: function release() {},
    wrap: function wrap(onComplete) {
        return onComplete;
    },
    write: function write() {}
});

var DefaultLogger = class_create(ILogger, {
    lock: function lock() {
    ...
    wrap: function wrap(onComplete) {
        ...
        return function (error) {
            ...
            if (error != null) {
                _this.write('stderr', error.message);
            }
            _this.release();
...
var NoLogger = class_create(DefaultLogger, {});

var FileLogger = class_create(DefaultLogger, {
    ...
    write: function write(type, str) {
        this.buffer.push(str);
    }
});

var CustomLogger = class_create(DefaultLogger, {
...
var _require = _src_class_Logger,
    createLogger = _require.create;
...
module.exports = function () {
	...
	_createClass(Compiler, [{
		...
		value: function process_(method, file, config, done) {
			this.logger.lock();
			var isAsync = done != null;
			if (isAsync) {
				done = this.logger.wrap(done);
			}
			var result = this[method](this.getContent_(file), file.uri.toLocalFile(), this.options, { file: file, method: method, config: config }, done);
			if (isAsync === false) {
				this.logger.release();
			}
			return result;
